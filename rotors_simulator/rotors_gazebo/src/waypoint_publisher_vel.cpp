...
class WaypointWithTime {
 public:
  ...
  WaypointWithTime(double t, float x, float y, float z, float vx, float vy, float vz, float _yaw)
      : position(x, y, z), velocity(vx, vy, vz), yaw(_yaw), waiting_time(t) {
  }
...
int main(int argc, char** argv) {
  ...
  if (args.size() != 2 && args.size() != 3) {
    ROS_ERROR("Usage: waypoint_publisher <waypoint_file>"
        "\nThe waypoint file should be structured as: space separated: wait_time [s] x[m] y[m] z[m] vx[m/s] vy[m/s] vz[m/s] yaw[deg])");
    return -1;
  }
  ...
  if (wp_file.is_open()) {
    ...
    while (wp_file >> t >> x >> y >> z >> vx >> vy >> vz >> yaw) {
      waypoints.push_back(WaypointWithTime(t, x, y, z, vx, vy, vz, yaw * DEG_2_RAD));
    }
    wp_file.close();
