...
void LeePositionController::SetOdometry(const EigenOdometry& odometry) {
  odometry_ = odometry;
  x_plot.push_back(odometry_.position(0));
  y_plot.push_back(odometry_.position(1));
  // plot_.update(x_plot, y_plot);
}

//New
void LeePositionController::SetWind(const EigenWind& wind) {
	wind_ = wind;
}
...
void LeePositionController::ComputeDesiredAcceleration(Eigen::Vector3d* acceleration) const {
  ...
  // Eigen::Vector3d Wind_force(5,0,0);
  //New
  Eigen::Vector3d Wind_force = 0.5*vehicle_parameters_.cda_*1.225*(wind_.velocity.cwiseProduct(wind_.velocity));

  // std::cout<<"Wind_force = ", Wind_force[0];
  // std::cout<<"CDA = "<<vehicle_parameters_.cda_;

  Eigen::Vector3d e_3(Eigen::Vector3d::UnitZ());

  *acceleration = (position_error.cwiseProduct(controller_parameters_.position_gain_)
      + velocity_error.cwiseProduct(controller_parameters_.velocity_gain_) + Wind_force) / vehicle_parameters_.mass_
      - vehicle_parameters_.gravity_ * e_3 - command_trajectory_.acceleration_W;

  //*acceleration = (Kp*position_error + Kd*position_error_derivative + Ki*position_error_intergral)/vehicle_parameters_.mass_;
}
