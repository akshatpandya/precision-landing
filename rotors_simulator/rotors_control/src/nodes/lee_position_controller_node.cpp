...
LeePositionControllerNode::LeePositionControllerNode(
  ...
  //New
  wind_sub = nh_.subscribe(mav_msgs::default_topics::WIND_SPEED, 1,
      &LeePositionControllerNode::WindCallback, this);
...
//New
void LeePositionControllerNode::WindCallback(const rotors_comm::WindSpeedConstPtr& wind_speed_msg){

  ROS_INFO_ONCE("LeePositionController got first wind message.");

  // ROS_INFO("%f", wind_speed_msg->velocity.x);
  EigenWind wind;
  windFromMsg(wind_speed_msg, &wind);
  lee_position_controller_.SetWind(wind);
}
